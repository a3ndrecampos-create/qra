...
class DispatchQueue {
    ...
    // Pedidos aguardando/chamando/em entrega — para a tela de acompanhamento do restaurante.
    private val _pedidosSnapshot = MutableStateFlow<List<PedidoEmAndamento>>(emptyList())
    val pedidosSnapshot: StateFlow<List<PedidoEmAndamento>> = _pedidosSnapshot
    ...
    private fun publish() {
        ...
        val pedidos = buildList {
            pendingOrders.forEach { order -> add(PedidoEmAndamento(order, PedidoStatus.AGUARDANDO)) }
            activeCallState.values.forEach { state ->
                val nome = motoboys[state.motoboyId]?.nome
                add(PedidoEmAndamento(state.order, PedidoStatus.CHAMANDO, state.motoboyId, nome))
            }
            activeDeliveryByMotoboy.forEach { (motoboyId, order) ->
                val nome = motoboys[motoboyId]?.nome
                add(PedidoEmAndamento(order, PedidoStatus.EM_ENTREGA, motoboyId, nome))
            }
        }.sortedBy { it.order.criadoEm }
        _pedidosSnapshot.update { pedidos }
    }
