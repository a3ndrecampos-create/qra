...
/** Status de um pedido do ponto de vista do restaurante (tela de acompanhamento). */
enum class PedidoStatus {
    AGUARDANDO,   // na fila, nenhum motoboy disponível no momento
    CHAMANDO,     // oferta enviada a um motoboy, aguardando resposta (ou timeout)
    EM_ENTREGA    // aceito, motoboy a caminho
}

data class PedidoEmAndamento(
    val order: OrderPayload,
    val status: PedidoStatus,
    val motoboyId: String? = null,
